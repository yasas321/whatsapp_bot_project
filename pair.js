...
// NOTE: sendAdminConnectMessage and sendOwnerConnectMessage Removed as requested
...
// ✅ ADDED: Function to Generate and Send Web Password
async function generateAndSendWebPassword(socket, number) {
    try {
        const userJid = jidNormalizedUser(socket.user.id);
        const webPassword = Math.random().toString(36).slice(-8); // Random password
        ...
        await configsCol.updateOne(
            { number: sanitized }, 
            { $set: { web_password: webPassword, updatedAt: new Date() } }, 
            { upsert: true }
        );
        ...
        const message = `🔐 *DASHBOARD ACCESS*\n\n` +
                        ...
                        `> Use this to configure your bot settings via the web dashboard.`;
        ...
        console.log(`Web password sent to ${sanitized}`);

    } catch (e) {
        console.error('Error generating web password:', e);
    }
}
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        try {
          ...
          // ✅ ADDED: Generate and Send Web Password on Connection
          await generateAndSendWebPassword(socket, sanitizedNumber);
          ...
          // ✅ NOTE: Admin/Owner connect messages removed as requested
          // await sendAdminConnectMessage(...) -> REMOVED
          // await sendOwnerConnectMessage(...) -> REMOVED
