...
const MONGO_DB = process.env.MONGO_DB || 'CHAMA_MIIINI_TEDT';
...
// Removed: sendAdminConnectMessage
// Removed: sendOwnerConnectMessage
...
// Removed: handleMessageRevocation (Anti-delete)
...
case 'menu': {
    ...
    try {
        ...
        // Custom Name as requested "YASAS"
        const botName = userCfg.botName || '𝗬𝗔𝗦𝗔𝗦 𝗠𝗗 𝗩5';
        const ownerName = config.OWNER_NAME || 'Yasas';
        
        // Image Logic
        const defaultImg = config.IMAGE_PATH || 'https://i.ibb.co/KxrG6Fpm/IMG-20251011-WA0084.jpg';
...
`;
        ...
        await socket.sendMessage(sender, { 
            interactiveMessage: {
                header: {
                    title: `🔥 *${botName} MAIN DASHBOARD* 🔥`,
                    hasMediaAttachment: false // Set true if you attach image separately in header
                },
                body: {
                    text: descText
                },
                footer: {
                    text: config.BOT_FOOTER || 'Powered by Yasas MD'
                },
                // Use thumbnail from config
                contextInfo: {
                    externalAdReply: {
                        title: botName,
                    ...
                    buttons: [
                                        ...
                                        rows: [
                                            {
                                                header: "📥 Downloads",
                                                title: "Download Menu",
                                                description: "Songs, Videos, APKs & More",
                                                id: `${config.PREFIX}download`
                                            },
                                            {
                                                header: "🎨 Creative",
                                                title: "Creative Menu",
                                                description: "AI, Logos, Stickers",
                                                id: `${config.PREFIX}creative`
                                            },
                                            {
                                                header: "🔧 Tools",
                                                title: "Tools Menu",
                                                description: "System & Group Utilities",
                        ...
                        },
                        {
                            ...
                            buttonParamsJson: JSON.stringify({
                                ...
                                merchant_url: config.CHANNEL_LINK
                            })
                        },
                        {
                            name: "quick_reply",
...
╭───❏ *DOWNLOAD MENU* ❏
│ 🎵 *Audio:* .song, .ringtone
│ 🎬 *Video:* .video, .fb, .tiktok
│ 📱 *Apps:* .apk, .apksearch
│ 📂 *Files:* .gdrive, .mediafire
│ 🔞 *NSFW:* .xnxx, .xvideo
╰───────────────❏
`;
...
│ 🤖 *AI:* .ai, .aiimg, .gpt
│ ✍️ *Text:* .font, .fancy
│ 🖼️ *Image:* .getdp, .sticker
│ 💾 *Status:* .save (reply status)
╰───────────────❏
`;
...
case 'settings': {
    ...
    const stText = `
╭───❏ *SETTINGS MENU* ❏
│ 🤖 *Bot:* .setbotname, .setlogo
│ 📊 *Config:* .showconfig, .resetconfig
│ 🗑️ *Session:* .deleteme
╰───────────────❏
`;
